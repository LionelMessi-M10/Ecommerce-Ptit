...
//     });
//   }, [id]);
 // Giả lập tình huống API không trả về dữ liệu
 fetchDataFromApi("/api/homeSideBanners")
 .then((res) => {
   if (Array.isArray(res) && res.length > 0) {
     setHomeSideBanners(res);
   } else {
     console.warn("API không trả về dữ liệu hợp lệ, sử dụng dữ liệu giả.");
     const mockData = [
       { subCatId: 1, catId: 2, images: ["https://via.placeholder.com/300x200?text=Banner+1"] },
       { subCatId: 2, catId: 3, images: ["https://via.placeholder.com/300x200?text=Banner+2"] }
     ];
     setHomeSideBanners(mockData);
   }
 })
 .catch(() => {
   console.error("Lỗi khi gọi API, sử dụng dữ liệu giả.");
   const mockData = [
     { subCatId: 1, catId: 2, images: ["https://via.placeholder.com/300x200?text=Banner+1"] },
     { subCatId: 2, catId: 3, images: ["https://via.placeholder.com/300x200?text=Banner+2"] }
   ];
